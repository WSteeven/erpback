...
class TransferenciasProductosEmpleadosExport extends DefaultValueBinder implements FromCollection, WithHeadings, WithStrictNullComparison, WithCustomValueBinder, WithColumnWidths, WithStyles, WithTitle, WithCustomStartCell
{
    use Exportable;
    ...
    public function styles(Worksheet $sheet)
    {
        ...
        $sheet->getStyle('A3:N3')->applyFromArray($textoTitulo);
        ...
        $sheet->getStyle('A4:N' . $totalFilas)->applyFromArray($textCenter);
        $sheet->getStyle('A4:N' . $totalFilas)->applyFromArray($bordeTabla);

        // Aplicar autoajuste de texto en celdas de datos
        $sheet->getStyle('A4:N' . $totalFilas)->getAlignment()->setWrapText(true);
        ...
        for ($row = 4; $row <= $totalFilas; $row++) {
            if ($row % 2 === 0) { // Fila par: fondo gris claro
                $sheet->getStyle('A' . $row . ':N' . $row)->applyFromArray([...$estiloGrisClaro]);
            }

            if ($row % 2 !== 0) { // Fila impar: fondo azul claro solo en columna E
                $sheet->getStyle('G' . $row)->applyFromArray($estiloAzulClaro);
            }
        }
    }
}
