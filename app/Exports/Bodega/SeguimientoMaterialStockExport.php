...
class SeguimientoMaterialStockExport extends DefaultValueBinder implements FromCollection, WithHeadings, WithStrictNullComparison, WithCustomValueBinder, WithColumnWidths, WithStyles, WithTitle, WithCustomStartCell
{
    use Exportable;
    ...
    public function styles(Worksheet $sheet)
    {
        ...
        $sheet->getStyle('A3:K3')->applyFromArray($textoTitulo);
        ...
        $sheet->getStyle('A4:K' . $totalFilas)->applyFromArray($textCenter);
        $sheet->getStyle('A4:K' . $totalFilas)->applyFromArray($bordeTabla);

        // Aplicar autoajuste de texto en celdas de datos
        $sheet->getStyle('A4:K' . $totalFilas)->getAlignment()->setWrapText(true);
        ...
        for ($row = 4; $row <= $totalFilas; $row++) {
            if ($row % 2 === 0) { // Fila par: fondo gris claro
                $sheet->getStyle('A' . $row . ':K' . $row)->applyFromArray([...$estiloGrisClaro]);
            }
