...
class GrupoController extends Controller
{
    ...
    public function listar()
    {
        ...
        if (request('ids_tareas')) {
            $arrayIdsTareas = explode(',', request('ids_tareas'));
            return Grupo::obtenerGruposRelacionadosConTareasConecel($arrayIdsTareas);
        }

        if (request('campos')) {
            return Grupo::ignoreRequest(['campos'])->filter()->latest()->get($campos);
        } else {
            return Grupo::filter()->latest()->get();
        }
    }
    ...
    {
        $results = GrupoResource::collection($this->listar());
