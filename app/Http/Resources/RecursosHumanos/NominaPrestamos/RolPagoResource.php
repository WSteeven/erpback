...
class RolPagoResource extends JsonResource
{
    ...
    public function toArray($request)
    {
        ...
        $modelo = [
            ...
            'salario' => number_format($this->empleado_info->salario, 2, ',', '.'),
            'dias' => $this->dias,
            'mes' => $this->mes,
            'anticipo' => number_format($this->anticipo, 2, ',', '.'),
            'iess' =>  number_format($this->iess, 2, ',', '.'),
            'sueldo' => number_format($this->sueldo, 2, ',', '.'),
            'Aporte IESS' => number_format($this->iess, 2),
            'SUPA' => $this->empleado_info->supa,
            'Extension de Cobertura de Salud' => number_format(ExtensionCoverturaSalud::where('empleado_id', $this->empleado_id)->where('mes', $this->mes)->sum('aporte'), 2),
            'Prestamo Hipotecario' => number_format(PrestamoHipotecario::where('empleado_id', $this->empleado_id)->where('mes', $this->mes)->sum('valor'), 2),
            'Prestamo Quirorafario' => number_format(PrestamoQuirorafario::where('empleado_id',$this->empleado_id)->where('mes', $this->mes)->sum('valor'), 2),
