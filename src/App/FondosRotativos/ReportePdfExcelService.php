<?php

namespace Src\App\FondosRotativos;

use App\Models\ConfiguracionGeneral;
use Barryvdh\DomPDF\Facade\Pdf;
use Exception;
use Illuminate\Http\Response;
use Illuminate\Support\Facades\Log;
use Maatwebsite\Excel\Facades\Excel;
use Symfony\Component\HttpFoundation\BinaryFileResponse;
use Throwable;

class ReportePdfExcelService
{
    public function __construct()
    {
    }

    /**
     * La función `imprimir_reporte` toma varios parámetros y, según el valor de `tipo_archivo`,
     * descarga un archivo Excel o transmite un archivo PDF.
     *
     * @param string $tipo_archivo La variable "tipo_archivo" representa el tipo de archivo a generar, como por
     * ejemplo "excel" o "pdf".
     * @param string $tamanio_pagina El parámetro "tamanio_pagina" hace referencia al tamaño, ya sea A4, A5, A3, etc.
     * @param string $orientacion_pagina El parámetro "orientacion_pagina" hace referencia a la orientación de
     * la página en el informe PDF. Los valores posibles son: vertical (portrait) o horizontal (landscape).
     * @param mixed $reportes Es un array que contiene los datos a incluir en el informe.
     * @param string $nombre_reporte El nombre del archivo de informe que se generará.
     * @param string $vista El parámetro "vista" es el nombre del archivo de vista que se utilizará para
     * generar el informe PDF. Generalmente es un archivo de plantilla Blade (.blade.php) que contiene
     * la estructura HTML y el estilo del informe.
     * @param object|null $export_excel El parámetro  es un objeto que representa los datos que
     * se exportarán a Excel. Se utiliza en el caso en que  (tipo de archivo) está
     * configurado en 'excel'. El método Excel::download() se llama con el objeto  y el
     *
     *
     * @return Response|BinaryFileResponse
     * del valor del parámetro ``.
     * @throws Exception
     */
    public function imprimirReporte(string $tipo_archivo, string $tamanio_pagina, string $orientacion_pagina, mixed $reportes, string $nombre_reporte, string $vista, object $export_excel = null)
    {
        try {
            $configuracion = ConfiguracionGeneral::first();
            $reportes['configuracion'] = $configuracion;
            $reportes['copyright'] = 'Esta informacion es propiedad de ' . $configuracion->razon_social . ' - Prohibida su divulgacion';
            switch ($tipo_archivo) {
                case 'excel':
                    return Excel::download($export_excel, $nombre_reporte . '.xlsx');
                case 'pdf':
                    $pdf = PDF::loadView($vista, $reportes);
                    $pdf->getDomPDF()->setCallbacks([
                        'totalPages' => true,
                    ]);
                    $pdf->setPaper($tamanio_pagina, $orientacion_pagina);

                    return $pdf->stream($nombre_reporte . '.pdf');
                default:
                    throw new Exception("No se puede imprimir el tipo de archivo");
            }
        } catch (Throwable $th) {
            Log::channel('testing')->info('Log', ['Error en imprimir_reporte', $th->getMessage(), $th->getCode(), $th->getLine()]);
            throw new Exception($th->getMessage() . '. [LINE CODE ERROR]: ' . $th->getLine());
        }
    }

    /**
     * @throws Exception
     */
    public function enviar_pdf($tamanio_pagina, $orientacion_pagina, $reportes, $vista)
    {
        $configuracion = ConfiguracionGeneral::first();
        $reportes['configuracion'] = $configuracion;
        $reportes['copyright'] = 'Esta informacion es propiedad de ' . $configuracion->razon_social . ' - Prohibida su divulgacion';
        $pdf = Pdf::loadView($vista, $reportes);
        $pdf->setPaper($tamanio_pagina, $orientacion_pagina);
        $pdf->setOption(['isRemoteEnabled' => true]);
        $pdf->render();
        //se genera el pdf
        return $pdf->output();
    }
}
